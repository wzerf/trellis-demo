...
function traverseTreeValues<T, V>(
    ...
    const dfs = (treeNode: T) => {
        ...
        result.push(value);
        const children = (treeNode as Record<string, unknown>)?.[childProps] as T[] | undefined;
        if (!children) {
            return;
        }
    ...
    const _filterTree = (nodes: T[]): T[] => {
        return nodes.filter((node: T) => {
            if (filter(node)) {
                ...
                if (children) {
                    node[childProps] = _filterTree(children) as T[keyof T];
                }
                return true;
            }
            return false;
...
function mapTree<T, V extends Record<string, unknown>>(
    ...
    return tree.map((node) => {
        ...
        const children = mapperNode[childProps] as T[] | undefined;
        if (children) {
            mapperNode[childProps] = mapTree(children, mapper, options);
        }
